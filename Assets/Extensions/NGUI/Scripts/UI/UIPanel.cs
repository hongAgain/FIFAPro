	...
	void FillAllDrawCalls ()
	{
		...
		for (int i = 0; i < widgets.Count; ++i)
		{
			...
			if (w.isVisible && w.hasVertices)
			{
				...
				Shader sd = w.shader;
			    if ((mGray && !(w is UILabel)) || w.gray)
			    {
			        int idx = sd.name.IndexOf("(");
			        string suffix = "";
			        if (idx != -1)
			        {
                        suffix = sd.name.Substring(idx);
                        sd = Shader.Find("GrayScale " + suffix);
			        }
			        else
			        {
                        sd = Shader.Find("GrayScale");
			        }
			    }
				...
				if (mat != null || sdr != null || tex != null)
				{
				    ...
				    if ((mGray && w is UILabel) || w.gray)
				    {
                        for (int j = 0; j < dc.cols.size; j++)
				        {
                            Color32 clr = dc.cols[j];
				            byte luminance = (byte)(clr.r * 0.22f + clr.g * 0.707f + clr.b * 0.071f);
                            clr.r = luminance;
                            clr.g = luminance;
                            clr.b = luminance;
				            dc.cols[j] = clr;
...
#if UNITY_EDITOR
    [ContextMenu("GrayTrue")]
    public void GrayTrue()
    {
        Gray(true);
    }

    [ContextMenu("GrayFalse")]
    public void GrayFalse()
    {
        Gray(false);
    }
#endif
