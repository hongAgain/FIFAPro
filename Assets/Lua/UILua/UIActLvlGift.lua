module("UIActLvlGift", package.seeall)
...
local itemTitleUI = "UILGRewardInfoTitle"
...
function InitRewardItem()
	rewardTable = {}
	local rewardConfig = Config.GetTemplate(Config.LevelGiftReward())
...
function OnShow()
	ActivityData.ReqLevelGiftInfo(OnGetRewardInfo)
end
function OnHide()
...
function OnGetRewardInfo( data )
	...
	for i,v in ipairs(rewardTable) do
		...
		UIRewardInfoItem.RefreshRewardItem(v,hasGot,canGet,lv)
	end
...
function OnGetRewardClick(go)
	...
	ActivityData.ReqLevelGiftReward(OnGetReward, param)
end
