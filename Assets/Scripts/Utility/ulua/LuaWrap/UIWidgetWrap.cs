...
public class UIWidgetWrap
{
	public static void Register(IntPtr L)
	{
		...
		LuaField[] fields = new LuaField[]
		{
			...
			new LuaField("gray", get_gray, set_gray),
	...
	static int get_gray(IntPtr L)
	{
		...
		if (obj == null)
		{
			...
			if (types == LuaTypes.LUA_TTABLE)
			{
				LuaDLL.luaL_error(L, "unknown member name gray");
			}
			else
			{
				LuaDLL.luaL_error(L, "attempt to index gray on a nil value");
			}
		}

		LuaScriptMgr.Push(L, obj.gray);
		return 1;
	}
	...
	static int set_gray(IntPtr L)
	{
		...
		obj.gray = LuaScriptMgr.GetBoolean(L, 3);
		return 0;
	}
