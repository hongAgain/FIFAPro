...
public static class ToLua 
{
    ...
    {
        Debuger.Log("Begin Generate lua Wrap for class {0}\r\n", className);
        sb = new StringBuilder();
    ...
    {
        string file = Application.dataPath + "/Scripts/Utility/ulua/LuaWrap/" + wrapClassName + "Wrap.cs";
    ...
    static void GenFunction()
    {
        ...
        for (int i = 0; i < methods.Length; i++)
        {
            ...
            if (m.IsGenericMethod)
            {
                Debuger.Log("Generic Method {0} cannot be export to lua", m.Name);
                continue;
            }
    ...
    static void CreateParamTypes(int pos, ParameterInfo[] p, bool beAddSelf)
    {
        ...
        {
            Debuger.LogError("CreateParamTypes i am not defined it: " + a.Count);
        }
    }
    ...
    static void NewIndexSetValue(Type t, string o, string name)
    {
        ...
        {
            Debuger.LogError("not defined type {0}", t);
        }
    }
