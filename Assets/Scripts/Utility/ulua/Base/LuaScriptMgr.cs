...
//#endif
...
#if !DEBUG
        DoString("DoFileWithAssert = dofile");
#endif
    ...
    {
        byte[] bt = null;
        name = string.Format("Lua/{0}", name);

#if UNITY_EDITOR
        string fullName = string.Format("{0}/{1}.lua", Application.dataPath, name);
        if (!File.Exists(fullName))
        {
            LogManager.Instance.LogError("Lua file is not exist: " + fullName);
            return null;
        }

        bt = File.ReadAllBytes(fullName);
#else
        bt = ResourceManager.Instance.LoadLua(name);
#endif
